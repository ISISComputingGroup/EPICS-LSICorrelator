...
class Records(Enum):
    ...
    ERRORMSG = Record("ERRORMSG", CHAR_PV_FIELDS)

    EXPERIMENTNAME = Record("EXPERIMENTNAME", CHAR_PV_FIELDS, has_setpoint=True)

    OUTPUTFILE = Record("OUTPUTFILE", CHAR_PV_FIELDS)

    START = Record("START", PARAM_FIELDS_BINARY, has_setpoint=True)

    STOP = Record("STOP", PARAM_FIELDS_BINARY, convert_from_pv=bool, has_setpoint=True)
    ...
    REPETITIONS = Record("REPETITIONS", INT_AS_FLOAT_PV, convert_from_pv=round, has_setpoint=True)

    CURRENT_REPETITION = Record("CURRENT_REPETITION", INT_AS_FLOAT_PV)

    RUNNING = Record("RUNNING", PARAM_FIELDS_BINARY, convert_from_pv=bool)

    TAKING_DATA = Record("TAKING_DATA", PARAM_FIELDS_BINARY, convert_from_pv=bool)

    WAITING = Record("WAITING", PARAM_FIELDS_BINARY, convert_from_pv=bool)

    WAIT_AT_START = Record(
        "WAIT_AT_START", PARAM_FIELDS_BINARY, convert_from_pv=bool, has_setpoint=True
    )

    CONNECTED = Record("CONNECTED", PARAM_FIELDS_BINARY, convert_from_pv=bool)

    SCATTERING_ANGLE = Record("SCATTERING_ANGLE", float_pv_with_unit("degree"), has_setpoint=True)

    SAMPLE_TEMP = Record("SAMPLE_TEMP", float_pv_with_unit("K"), has_setpoint=True)

    SOLVENT_VISCOSITY = Record("SOLVENT_VISCOSITY", float_pv_with_unit("mPas"), has_setpoint=True)

    SOLVENT_REFRACTIVE_INDEX = Record(
        "SOLVENT_REFRACTIVE_INDEX", float_pv_with_unit(""), has_setpoint=True
    )

    LASER_WAVELENGTH = Record("LASER_WAVELENGTH", float_pv_with_unit("nm"), has_setpoint=True)

    SIM = Record("SIM", {"type": "enum", "enums": ["NO", "YES"]}, convert_from_pv=bool)

    DISABLE = Record("DISABLE", {"type": "enum", "enums": ["NO", "YES"]}, convert_from_pv=bool)
